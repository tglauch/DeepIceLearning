...
def dense_block(x0, n=8):
    ...
    for i in range(n):
        ...
        x0 = concatenate([x0, x], axis=-1)
    return x


def inception_unit(nfilters, x0):
    x1 = Convolution3D(
        nfilters, (1, 1, 1), padding='same', activation='relu')(x0)
    x1 = Convolution3D(
        nfilters, (3, 3, 3), strides=(1, 1, 2),padding='same', activation='relu')(x1)

    x2 = Convolution3D(
        nfilters, (1, 1, 1), padding='same', activation='relu')(x0)
    x2 = Convolution3D(
        nfilters, (5, 5, 5), padding='same', strides=(1, 1, 2), activation='relu')(x2)

    x3 = Convolution3D(
        nfilters, (7, 7, 7), padding='same', strides=(1, 1, 2),activation='relu')(x0)

    x4 = MaxPooling3D(
        (3, 3, 3), strides=(1, 1, 3), padding='same')(x0)
    x4 = Convolution3D(
