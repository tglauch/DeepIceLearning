...
if __name__ == "__main__":
    ...
    model_settings, model_def = parse_config_file(
        os.path.join(DATA_DIR, 'model.cfg'))
    shapes, shape_names, inp_variables, inp_transformations,\
    ...
    if args.__dict__['final']:
        model.load_weights(
            os.path.join(DATA_DIR, 'final_network.h5'))
    else:
        model.load_weights(
            os.path.join(DATA_DIR, 'best_val_loss.npy'))
    res = []
    test_out = []
    ...
    file_len = read_input_len_shapes(
        mc_location, input_files)
    ...
    file_handlers = [h5py.File(
        os.path.join(mc_location, file_name), 'r')
        for file_name in input_files]
    prediction = model.predict_generator(
        generator(args.batch_size, file_handlers,
                  test_inds, shapes, inp_variables,
