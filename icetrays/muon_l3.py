...
def cuts(phy_frame):
    ...
    """
    if np.degrees(phy_frame['SplineMPE'].dir.zenith) < 80:
        return False
    else:
        return True
...
generator_co = 97852 * low_e + 98717 * high_e
flux = GaisserH4a()

def corsika_weight(phy_frame):
    if 'I3MCWeightDict' in phy_frame:
        return
    energy = phy_frame['MCPrimary1'].energy
    ptype = phy_frame['MCPrimary1'].pdg_encoding
    weight = flux(energy, ptype)/generator_co(energy, ptype)
...
#                   Streams=[icetray.I3Frame.Physics])
    ...
    tray.AddModule(corsika_weight, 'weighting',
                   Streams=[icetray.I3Frame.Physics])
    tray.AddModule(reco_q.classify_wrapper, "classifiy",
                   surface=surface,
                    Streams=[icetray.I3Frame.Physics])
