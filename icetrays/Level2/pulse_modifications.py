...
import numpy as np
import copy
import pulse_modification_functions
...
class PulseModification(icetray.I3ConditionalModule):
    ...
    def Configure(self):
        ...
        self._out_key = self.GetParameter("OutKey")
    ...
    def Calibration(self, frame):
        ...
        for omkey, calib in frame['I3Calibration'].dom_cal.items():
            if (omkey.om < 61 and omkey.om > 0 and omkey.string > 0 and
                    omkey.string < 87):
    ...
    def Physics(self, frame):
        ...
        if self._out_key is None:
            ...
            if self._pulse_key + 'TimeRange' in frame.keys():
                frame[self._out_key + '_modTimeRange'] = dataclasses.I3TimeWindow(
                                                frame[self._pulse_key + 'TimeRange'])
